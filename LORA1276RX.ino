...
void printPacketInfo() {
      unsigned char payload_size = SPIreadRegister(LR_RegRxNbBytes);	// read packet size
      SPIreadBurst(0x00, rxbuf, payload_size);	        // read from fifo
      unsigned char snr = SPIreadRegister(LR_RegPktSnrValue);
      unsigned char rssi = SPIreadRegister(LR_RegPktRssiValue);
